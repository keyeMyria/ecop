...
Ext.define('Ecop.view.item.ItemDescription', {
    ...
    dockedItems: {
        ...
        items: [{
            xtype: 'component',
            html: '<b style="color:red;">注意事项:</b> 商品描述图宽度建议为790,大于790的将自动缩小为790。图片大小不得超过2MB,高度和宽度之比不得超过2，格式可以是gif/png/jpg。'
        }]
    },
    ...
    items: [{
        ...
        items: [{
            ...
            columns: [{
                ...
                renderer: function (v, c, record) {
                    return record.get('format') ? record.get('text') + '.' + record.get('format') : record.get('text');
                },
                flex: 1
            }]
        }, {
        ...
        bbar: [{
            ...
            iconCls: 'x-fa fa-plus-circle',
            tooltip: '在当前位置添加新图片',
            afterTpl: Ecop.util.Util.imageInputTpl.join('').replace('image/png', 'image/png, .psd'),
            text: '图片',
            listeners: {
