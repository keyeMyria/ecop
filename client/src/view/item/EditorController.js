Ext.define('Ecop.view.item.EditorController', {
    ...
    createForm: function (record) {
        var me = this, view = me.getView(), vp = view.up('viewport');
            ...
            viewModel: {
                ...
                formulas: {
                    showRight: function (get) {
                        var s = get('images'), record = get('selectedImage');
    ...
    onContextMenu: function (table, record, tr, rowIndex, e) {
                ...
                viewModel: {
                    ...
                    formulas: {
                        hideOffline: function (get) {
                            var item = get('item');
                            return (!Ecop.auth.hasPermission('item.inactivate') && item.get('isInactive')) || item.get('isOffline');
                        }
                    }
                },
        ...
        Ext.defer(function() {
            var i, items = menu.query('menuitem');
            ...
            for (i=0; i<items.length; i++) {
                if (!items[i].isHidden()) {
                    ...
                    menu.down('#qrcode') && menu.down('#qrcode').el.down('a').set({
                        href: '/itemqr?itemId=' + record.get('itemId')
                    });
                    menu.down('#openitem') && menu.down('#openitem').el.down('a').set({
    ...
    onContextMenuClick: function (menu, item, e) {
        var me = this, grid = me.getView(), status, menuId = item.getItemId();
    ...
    onBtnSave: function () {
        var s = this.getView().getStore(), modified = [];
    ...
    onItemSave: function () {
        var me = this, boms = [], images = [],
            item = me.getCurrentItem(), params = item.getChanges(),
    ...
    beforeItemWinDestroy: function () {
        var me = this, item = me.getCurrentItem();
    ...
    afterItemFormRender: function () {
        var me = this, f = me.dialog, item = me.getCurrentItem();
        me.bomOrderChanged = false; me.imageModified = false;
    ...
    onSkuAdd: function () {
        var me = this; f = me.dialog, sku = f.down('skuinput').getValue(),
    ...
    onImageAdd: function (fileBtn) {
        var me = this, store = me.dialog.getViewModel().get('images'),
    ...
    onImageUpdate: function (fileBtn) {
        ...
        me.addOrUpdateImage(file, 'item', image.getId())
    ...
    onImageMoveLeft: function () {
        var me = this, vm = me.dialog.getViewModel(), idx,
            s = vm.get('images'), image = vm.get('selectedImage');

        me.doImageMove(image, s.indexOf(image) - 1);
    },

    onImageMoveRight: function () {
        var me = this, vm = me.dialog.getViewModel(),
            s = vm.get('images'), image = vm.get('selectedImage');
    ...
    onImageDownload: function () {
        var me = this, image = me.lookup('imageList').selection;
    ...
    onDescBtnAddModule: function () {
        var me = this,
            list = me.lookup('moduleType'), store = list.getStore();
    ...
    onModuleTypeChoice: function (list, record) {
        var me = this, root = me.lookup('moduleTree').getRootNode(),
            all = list.getStore().getData().getSource(),
            idx = all.indexOfKey(record.getId()), i,
            node = {
                'id': record.get('id'),
    ...
    onDescImageAdd: function (fileBtn) {
        var me = this, tree = me.lookup('moduleTree'),
            file = fileBtn.el.down('input').dom.files[0],
            rec = tree.selection;
        ...
        me.addOrUpdateImage(file, 'desc').then(function(image){
            ...
            if (rec.get('rid')) { // this is a resource
    ...
    onDescCopy: function () {
        var me = this;
        Ext.Msg.prompt('输入商品号/商品链接',
            '请输入大管家商品号或天猫商品链接。请注意当前全部描述均会被覆盖！',
            function (btnId, text) {
                var itemId;
                if (btnId === 'ok' && text) {
    ...
    onDescTreeSelect: function (model, record) {
        var me = this, cmp = me.lookup('preview');
    ...
    onDescPreview: function () {
        var me = this, cmp = me.lookup('preview'), html = '';
        me.lookup('moduleTree').getRootNode().eachChild(
            function (module) {
