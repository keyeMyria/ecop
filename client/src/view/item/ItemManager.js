Ext.define('Ecop.view.item.ItemManager', {
    ...
    requires: [
        'Ecop.view.category.CategoryManager',
        'Ecop.view.item.EditorGrid'
    ],
    ...
    initComponent: function () {
        ...
        me.items = me.catman ? [me.catman, {xtype: 'splitter'}, me.itemman] : [me.itemman];
    ...
    afterTreeRender: function (tree) {
        ...
        me.itcDropTarget = new Ext.dd.DropZone(tree.getEl(), {
            ...
            onNodeOver: function (node, source, e, data) {
                if (Ext.fly(node).hasCls(Ext.baseCSSPrefix + 'grid-tree-node-leaf') &&
                    tree.getView().getRecord(node).get('id') !== data.records[0].get('primaryCategoryId')
                ) {
            ...
            onNodeDrop: function (node, source, e, data) {
                var items = [], rec = tree.getView().getRecord(node),
                    newCatId = rec.get('id');

                // check whether a drop is possible, see onNodeOver
                if (!rec.isLeaf() || newCatId == data.records[0].get('primaryCategoryId')){
                    return false;
                }
