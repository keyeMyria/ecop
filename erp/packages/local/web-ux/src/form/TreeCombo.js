...
If store root note has 'checked' property tree combo becomes multiselect combo
(tree store must have records with 'checked' property)

Options:

selectChildren - if set true and if store isn't multiselect, clicking on an
    non-leaf node selects all it's children
canSelectFolders - if set true and store isn't multiselect clicking on a folder
    selects that folder also as a value
...
Ext.define('Web.ux.form.TreeCombo', {
    ...
    addRecRecord: function (record) {
        ...
        for (i = 0; i < me.recursiveRecords.length; i++) {
            item = me.recursiveRecords[i];
            if (item) {
    ...
    setValue: function (valueInit) {
        ...
        Ext.each(me.recursiveRecords, function (record) {
            var id = record.get(me.valueField),
                index = values.indexOf(String(id));
    ...
    checkParentNodes: function (node) {
        ...
        node.eachChild(function (nodesingle) {
            var id = nodesingle.getId(),
                index = me.ids.indexOf(String(id));
    ...
    initComponent: function () {
        ...
        me.tree = Ext.create('Ext.tree.Panel', {
            hidden: true,
            minHeight: 300,
            rootVisible: (me.rootVisible !== undefined) ? me.rootVisible : true,
            floating: true,
            useArrows: true,
            width: me.treeWidth,
    ...
    addIds: function (record) {
        ...
        if (me.ids.indexOf(String(record.getId())) === -1) {
            me.ids.push(String(record.get(me.valueField)));
        }
    },

    removeIds: function (record) {
        var me = this,
            index = me.ids.indexOf(String(record.getId()));
    ...
    addRecord: function (record) {
        ...
        for (var i=0,j=me.records.length;i<j;i++) {
            ...
            if (item && item.getId() == record.getId()) {return; }
        }
